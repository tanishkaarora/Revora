...
class GroqProvider(LLMProvider):
    def __init__(self, api_key: Optional[str] = None, model: str = "llama-3.1-8b-instant"):
        ...
        self.async_client = AsyncGroq(api_key=self.api_key) if self.api_key else None

    async def _call_groq_json_async(self, prompt: str, system_prompt: str = "") -> Optional[dict]:
        ...
        try:
            ...
            logger.warning(f"AsyncGroq API JSON call failed, using graceful fallback: {e}")
        return None

    async def _call_groq_text_async(self, prompt: str, system_prompt: str = "") -> Optional[str]:
        if not self.async_client:
            return None
        try:
            messages = []
            ...
            logger.warning(f"AsyncGroq API text call failed, using graceful fallback: {e}")
        return None

    def _call_groq_json_sync(self, prompt: str, system_prompt: str = "") -> Optional[dict]:
        if not self.client:
            return None
        try:
            messages = []
    ...
    def _call_groq_text_sync(self, prompt: str, system_prompt: str = "") -> Optional[str]:
        if not self.client:
            return None
        try:
            messages = []
    ...
    async def aclassify_failure_cause(self, error_code: str, error_reason: str) -> Tuple[FailureCause, float]:
        system_prompt = (
        ...
        response_json = await self._call_groq_json_async(prompt, system_prompt)
    ...
    def classify_failure_cause(self, error_code: str, error_reason: str) -> Tuple[FailureCause, float]:
        ...
        response_json = self._call_groq_json_sync(prompt, system_prompt)
        if response_json and "cause" in response_json:
            cause = response_json["cause"]
            confidence = float(response_json.get("confidence", 0.8))
            valid_causes = ["bank_timeout", "insufficient_balance", "expired_mandate", "wrong_otp", "card_declined", "unknown"]
    ...
    async def agenerate_hinglish_message(self, customer_name: str, amount_rupees: float, payment_link: str, cause: FailureCause) -> str:
        system_prompt = (
        ...
        message = await self._call_groq_text_async(prompt, system_prompt)
        if message:
            return message
    ...
    def generate_hinglish_message(self, customer_name: str, amount_rupees: float, payment_link: str, cause: FailureCause) -> str:
        ...
        message = self._call_groq_text_sync(prompt, system_prompt)
        if message:
            return message
            
        return fallback_generate_hinglish_message(customer_name, amount_rupees, payment_link, cause)

    async def aextract_commitment(self, reply: str) -> Tuple[bool, Optional[str], float, str]:
        system_prompt = (
